---
title: "THE LIBERAL PARTY WILL NOT WIN IN THE 2019 CANADIAN FEDERAL ELECTION ANYMORE IF EVERYONE HAD VOTED"
author: "Jiayi YANG"
...
To accomplish this goal, we do analysis based on the data from Canadian Election Survey (CES) and Stat Canada 2016 Education Census. We will firstly build a logistic regression by Bayesian approach. And then, we will also utilize the multilevel regression model with post-stratification. We analyzed individual-level survey data and post-stratified census data to analyze the effects caused by key demographic variables on the voting decision and predict the probability of Justin Trudeau still winning the election. Our prediction suggests that, as the voter has been expanded to all the citizen, Trudeau will probabily not be elected any more.
...
```
For education level of respondent, it did not show a distinct relationship with voting preference for the 2019 Canadian Federal election. Across all education levels, the Liberal party do not have the most number of voting.  
...
The census data we used was the 2017 General Social Survey (GSS) on the family. It is a nationwide survey which was designed to social-demographic variables of Canadians and capture social-social demographic and social-economic trends.Its target population includes all non-institutionalized person with age over 15, living in the 10 chosen provinces inside Canada @PS3. So, it could be a reliable census data which support to be a study population. However, the sample size for this database is really small which only has around 4894 observations.
